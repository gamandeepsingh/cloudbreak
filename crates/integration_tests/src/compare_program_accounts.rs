...
#[derive(Parser, Debug)]
#[command(name = "compare_responses")]
#[command(about = "Compare Solana program accounts from two RPC endpoints")]
pub struct Args {
...
pub struct Config {
    /// First (tested) RPC endpoint
    pub rpc1: RpcEndpoint,

    /// Second (healthy) RPC endpoint
    pub rpc2: RpcEndpoint,

    /// The program IDs to get accounts for
...
async fn get_last_tx_time(
    ...
    let result = async {
        ...
        // let rpc_response: RpcResponse<Vec<SignatureInfo>> = serde_json::from_value(raw)?;
...
pub async fn run(args: &Args) -> Result<()> {
    ...
    for program_id in &config.program_ids {
        ...
        let (pubkeys1, pubkeys2) = tokio::join!(
            get_pubkeys(&client, &config.rpc1.url, program_id),
            get_pubkeys(&client, &config.rpc2.url, program_id),
        );
        ...
        let only_in_rpc1: Vec<String> = set1
            .difference(&set2)
            .map(|s| (*s).clone())
            .collect::<Vec<_>>()
            .into_iter()
            .collect();
        let mut only_in_rpc1 = only_in_rpc1;
        only_in_rpc1.sort();

        // ------------------------------------------------------------
        // load only_in_rpc1 from file
        let previous_only_in_rpc1: Vec<String> = serde_json::from_str(&std::fs::read_to_string(
            "crates/integration_tests/compare_responses_results/only_in_rpc1.json",
        )?)?;

        // Find keys in BOTH vectors
        let current_set: HashSet<_> = only_in_rpc1.iter().collect();
        let previous_set: HashSet<_> = previous_only_in_rpc1.iter().collect();

        // let in_both: Vec<&String> = current_set.intersection(&previous_set).cloned().collect();
        // Keys only in current run (new)
        let only_new: Vec<_> = current_set.difference(&previous_set).collect();

        // Keys only in previous run (disappeared)
        ...
        let only_in_rpc2: Vec<String> = set2
            .difference(&set1)
            .map(|s| (*s).clone())
            .collect::<Vec<_>>()
            .into_iter()
            .collect();
        let mut only_in_rpc2 = only_in_rpc2;
        only_in_rpc2.sort();
...
async fn find_keys_in_db(
    ...
    // Convert base58 pubkeys to hex-encoded bytea literals for SQL
    ...
    // Build the SQL query with array of bytea values
    let sql = format!(
    ...
    // Convert results back to base58 strings
